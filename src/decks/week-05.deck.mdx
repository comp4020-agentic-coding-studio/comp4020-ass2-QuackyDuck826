...
## Wiring an actuator, step by step

1. **LED** — add a current-limiting resistor in series before power ever
   goes on; without one it draws whatever the pin will give it and burns
   out in seconds.
2. **Motor** — drive it through a transistor or H-bridge, never straight
   off a pin; a pin supplies a few tens of milliamps, a motor wants far
   more, so the driver switches a separate supply instead.
3. **Servo** — power, ground, and one PWM signal pin; the width of the
   pulse on that pin sets the angle, so the code side is close to one
   line once it's wired correctly.
4. **Speaker (piezo)** — a digital pin through a small resistor is
   enough, no driver needed — the quickest actuator here for an early
   win.
5. Get a tutor to check the wiring before powering anything that draws
